...
export async function renderAccountingPdf(model: AccountingPdfModel): Promise<Buffer> {
  ...
  if (model.company.logoUrl && model.company.logoUrl.startsWith("data:image")) {
    try {
      ...
      doc.addImage(model.company.logoUrl, fmt, rtl ? W - M - 22 : M, 4, 22, 20);
      logoDrawn = true;
  ...
  doc.setFontSize(15);
  const nameX = logoDrawn ? (rtl ? W - M - 26 : M + 26) : startX;
  ...
  if (model.columns && model.rows) {
    y = drawTable(doc, model.columns, model.rows, y, W, M, rtl, sanitize);
  }
  ...
  if (model.paymentTerms) { doc.setFont("helvetica", "bold"); label(model.paymentTerms, y); y += 5; }
  if (model.notes) {
    doc.setFont("helvetica", "normal");
  ...
  if (model.badge && (model.badge.kind === "draft" || model.badge.kind === "void")) {
    doc.setTextColor(226, 232, 240); doc.setFont("helvetica", "bold"); doc.setFontSize(72);
...
function drawTable(doc: any, columns: PdfColumn[], rows: Record<string, string>[], startY: number, W: number, M: number, rtl: boolean, sanitize: (v: unknown) => string): number {
  const usable = W - 2 * M;
