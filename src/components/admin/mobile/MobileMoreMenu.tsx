...
import { Bell, LogOut, ChevronRight, ChevronLeft, Clock, FileText, Receipt, FileStack, Share2 } from 'lucide-react';
 ...
 */
export default function MobileMoreMenu({ lang, isRtl, tabs, onSelectTab, unreadNotifications, onOpenNotifications, onLogout }: MobileMoreMenuProps) {
  const ChevronIcon = isRtl ? ChevronLeft : ChevronRight;

  return (
    <div className="lg:hidden pb-24">
      <h2 className="text-lg font-extrabold text-slate-900 mb-4 px-1">{TITLE[lang] ?? TITLE.en}</h2>

      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden divide-y divide-slate-100 mb-4">
        <button
          type="button"
          onClick={onOpenNotifications}
          className="w-full flex items-center gap-3 px-4 py-3.5 min-h-[44px] text-start cursor-pointer bg-transparent border-0"
        >
          <span className="w-9 h-9 shrink-0 flex items-center justify-center bg-orange-50 text-orange-500 rounded-lg">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3.5 min-h-[44px] text-start cursor-pointer bg-transparent border-0"
            >
              <span className="w-9 h-9 shrink-0 flex items-center justify-center bg-slate-100 text-slate-600 rounded-lg">
        ...
        {COMING_SOON_ITEMS.map((item) => {
          ...
          return (
            <div
              key={item.key}
              aria-disabled="true"
              className="w-full flex items-center gap-3 px-4 py-3.5 min-h-[44px] opacity-50 cursor-not-allowed select-none"
            >
              <span className="w-9 h-9 shrink-0 flex items-center justify-center bg-slate-100 text-slate-400 rounded-lg">
                <Icon className="w-4.5 h-4.5" />
              </span>
      ...
      {onLogout && (
        <button
          type="button"
          onClick={onLogout}
          className="w-full flex items-center justify-center gap-2 px-4 py-3.5 min-h-[44px] bg-white hover:bg-red-50 border border-red-200 text-red-500 font-bold text-sm rounded-xl cursor-pointer shadow-sm transition-colors"
        >
