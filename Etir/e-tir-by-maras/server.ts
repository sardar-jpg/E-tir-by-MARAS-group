...
async function updateDoc(docRef: any, data: any) {
  if (useMemoryFallback) {
    return handleUpdateDocMemory(docRef, data);
  }
  try {
    return await rawUpdateDoc(docRef, data);
  } catch (error) {
    console.warn("Firestore updateDoc failed. Switching to robust Memory Fallback.", error);
    useMemoryFallback = true;
    return handleUpdateDocMemory(docRef, data);
  }
}
...
async function deleteDoc(docRef: any) {
  ...
  try {
    ...
    console.warn("Firestore deleteDoc failed. Switching to robust Memory Fallback.", error);
    useMemoryFallback = true;
    return handleDeleteDocMemory(docRef);
  }
}
...
async function addDoc(colRef: any, data: any) {
  if (useMemoryFallback) {
    return handleAddDocMemory(colRef, data);
  }
  try {
    return await rawAddDoc(colRef, data);
  } catch (error) {
    console.warn("Firestore addDoc failed. Switching to robust Memory Fallback.", error);
    useMemoryFallback = true;
    return handleAddDocMemory(colRef, data);
  }
}
...
try {
  ...
  if (firebaseConfig) {
    ...
    db = getFirestore(firebaseApp, initialId);
  } else {
...
async function testConnection() {
  ...
  try {
    await rawGetDoc(doc(db, "test", "connection"));
...
async function startServer() {
  ...
  // Custom CORS middleware to support external frontends (e.g. etir.app) querying this backend
  app.use((req, res, next) => {
  ...
  // Validate database connection and run fallbacks if necessary
  ...
  app.put("/api/drivers/:id", async (req, res) => {
    try {
      ...
      const updatedDriver: Driver = {
        ...
        username: username !== undefined ? username : original.username,
        truckNumber: truckNumber !== undefined ? truckNumber : original.truckNumber,
        phone: phone !== undefined ? phone : original.phone,
        truckType: truckType !== undefined ? truckType : original.truckType,
        latitude: latitude !== undefined ? latitude : original.latitude,
        longitude: longitude !== undefined ? longitude : original.longitude,
        lastUpdated: lastUpdated !== undefined ? lastUpdated : original.lastUpdated,
        avatarUrl: avatarUrl !== undefined ? avatarUrl : original.avatarUrl,
      };
