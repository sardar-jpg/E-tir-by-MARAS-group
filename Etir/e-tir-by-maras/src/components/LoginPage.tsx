...
import { Ship, Globe, User, Lock, Phone, Truck, Shield, ClipboardSignature, KeyRound } from "lucide-react";
...
interface LoginPageProps {
  ...
  onLoginSuccess: (session: { role: "admin" | "driver"; email?: string; driver?: Driver | null; loginType?: "firebase" | "local" }) => void;
  onViewPrivacy?: () => void;
  onViewTerms?: () => void;
}
...
export default function LoginPage({ lang, onSetLang, onLoginSuccess, onViewPrivacy, onViewTerms }: LoginPageProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const enteredEmail = loginUsername.includes("@") 
        ? loginUsername.trim().toLowerCase() 
        : `${loginUsername.trim().toLowerCase()}@e-tir.com`;
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!regFullName.trim() || !regUsername.trim() || !regPhone.trim() || !regTruckId.trim() || !regPassword) {
    ...
    try {
      ...
      const email = regUsername.includes("@") ? regUsername.trim() : `${regUsername.trim().toLowerCase()}@e-tir.com`;
      ...
      <div className="w-full max-w-md bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl z-10 transition-all">
        ...
        {!isRegisterMode ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-1.5 bg-slate-900 border border-slate-800/80 p-1 rounded-xl">
              <button
                type="button"
                onClick={() => {
                  setLoginRole("admin");
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    required
                    placeholder={loginRole === "admin" ? "e.g. sardar@maras.iq" : "e.g. ihab"}
                    value={loginUsername}
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 bg-slate-900 hover:bg-slate-850 hover:text-white text-slate-300 font-bold text-xs rounded-xl border border-slate-800 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md"
              >
