...
- **Validation** (`sanitizeNotificationPreferencesUpdate`):
  ...
  - `security_system_alerts: false` is **not** a validation error — it's
    silently ignored (the field is always forced back to `true`), so a
    request that also changes other categories in the same call still
    succeeds for those. There is no way to make this endpoint persist
    `security_system_alerts: false` under any input.
- On success, persists the full resolved preferences document (`id`,
  every category, `updatedAt`) and responds
  `{ preferences: AdminNotificationPreferences }` (the actual saved
  state, reflecting any ignored/forced values).
...
- Each of the 7 non-security categories renders as a real, accessible
  ...
  with just that one category's new value; the switch optimistically
  flips, shows a small per-card "Saving…" spinner while the request is in
  flight, and **reverts back to its previous state plus an inline error
  banner if the request fails** — a failed save is never left looking
  like it succeeded.
