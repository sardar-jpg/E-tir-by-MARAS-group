...
- **Cost statement write access for accounts admins** — found during PR #58, read/display completeness fixed in PR #60 (`feature/accounts-cost-statements-data-completeness`), write access still open: `POST /api/cost-statements/:shipmentId` uses `requireFullAdmin` (super/operation only), so an accounts admin — the type the 'costs' tab is shown to for exactly this purpose — currently cannot save a cost statement from the UI (the request 403s), including on shipments that don't have one yet ("Add Costs" never appears for them — see the PR #60 item below). Needs a product decision, not a default-widen:
  - **Option A** — accounts admins can create/update cost statements (`requireFullAdmin` → a new `canWriteCostStatements` allowing `super`/`accounts`).
  - **Option B** — accounts admins stay view/export-only; statements are always created by super/operation.
  - **Option C** — accounts admins may edit payment/accounting fields only (`paidAmount`/`notes`/payment status), not the cost-item breakdown.
