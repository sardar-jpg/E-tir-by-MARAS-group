...
- **"Logistics Command Hub" header shows a static owner identity for every admin** — noticed during the PR #76 sidebar review: the Dashboard hero card's subtitle (`MARAS Cargo HQ · sardar@maras.iq (Senior Administrator)`) is a hardcoded string, not the currently logged-in admin — an Operation or Accounts admin sees the owner's name/email there too. Not a security issue (no data is exposed beyond what's already static company branding) and out of scope to redesign here, but worth deciding whether it should read the logged-in admin's own identity instead, since it reads as slightly misleading (Part 4 "dashboard honesty" territory).
