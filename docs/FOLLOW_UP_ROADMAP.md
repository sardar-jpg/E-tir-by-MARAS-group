...
- ~~**Repository Cleanup / Legacy Files Review**~~ — **Done** in the `chore/safe-code-cleanup-naming-audit` cleanup pass. Removed `Etir/e-tir-by-maras` and `etir-new` (both pre-flagged here for removal), plus `assets/.aistudio/`, `metadata.json`, and `firebase-blueprint.json` — leftover Google AI Studio scaffold artifacts confirmed to have zero references anywhere in the build/source. See `docs/CODE_CLEANUP_AUDIT.md` for the full audit.
